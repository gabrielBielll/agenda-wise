...
| `a004-comissao` | [0158](0158-duna-para-orla-e-vale-a004-integrada-e-colisao-de-migration.md) | 🔄 A-004 reaplicada no topo canônico: modalidades percentual/fixa, snapshot por sessão e transferência mensal em lote. Colisão de IDs removida: remuneração `080000`, OAuth `090000`. Teste sem banco verde; JVM encerrada antes do resumo da suíte com banco | **vale** (revisar front) · **orla** (revisar regra e migration) |
...
| `fase-1-front` | [0156](0156-duna-para-orla-e-vale-state-oauth-validado.md) | 🔄 `state` OAuth agora é opaco, armazenado só como SHA-256 por clínica/usuário, expira em 10 minutos e é consumido atomicamente antes da troca com o Google. 126 testes/436 asserções verdes no PostgreSQL 18 | **vale** (revisar callback e contrato com o front) · **orla** (receber) |
