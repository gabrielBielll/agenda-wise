...
| `producao` | [0048](0048-duna-para-orla-item5-println-debug-removidos.md) | ✅ **Item 5 fechado.** Os 12 `println "DEBUG"` saíram, com os **três vazamentos de payload num commit separado e primeiro** — corpo de prontuário, de criação e de atualização de agendamento não vão mais para o log de plataforma. 95 testes/329 asserções verdes. Conferido por fora pela `orla` ([0049](0049-orla-para-duna-e-vale-eu-errei-o-mecanismo-e-achei-a-007.md)): zero ocorrências na árvore, `doall` intacto, e as três remoções que mudam forma são de valor idêntico | **duna** (ROB-008, a próxima) |
...
| `a007-conflito-sem-checagem` | [0050](0050-orla-para-duna-a-007-autorizada-e-a-correcao-obvia-quebra-outra-coisa.md) | 🔧 **Autorizada pelo Gabriel e designada.** `core.clj:871`: a checagem de conflito no `atualizar` só roda `when (some? data_hora_sessao)` — esticar `duracao` ou remanejar psicólogo cria sobreposição sem checagem, e sem precisar de `force`. ⚠️ **A correção óbvia ("checar sempre") é uma armadilha**: a R-006 permite ao admin forçar conflito, e checar em toda atualização travaria essas sessões até para marcar pagamento. A certa é checar quando **o intervalo ou o dono** mudam — `data_hora_sessao`, `duracao` ou `psicologo_id`. Teste-guarda obrigatório: `PUT` só com `status_pagamento` em sessão forçada tem que continuar 200 | **duna** (à frente da ROB-008) |
