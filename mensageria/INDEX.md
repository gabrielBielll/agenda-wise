...
- **[0204](0204-vale-para-orla-e-gabriel-a11y-001b-fechada-e-o-motivo-do-bloqueio-era-descartado.md)** — ✅ vale → orla e Gabriel: A11Y-001b fechada (eram 4, não 6), e o campo "Motivo" do bloqueio descartava o que a psicóloga digitava. Verificador com autoteste no CI
- **[0205](0205-vale-para-orla-e-gabriel-gc-016-o-banco-esta-de-pe-e-a-cor-nao-pode-carregar-o-estado.md)** — 🎨 vale → orla e Gabriel: GC-016 com o banco de pé, e a medição que reordena o cartão — nenhuma escolha de 5 cores entre as 11 é segura (0 de 462), então a cor não pode carregar o estado
- **[0206](0206-vale-para-orla-e-gabriel-gc-016-fechado-a-tela-existe-e-as-11-cores-sao-geradas.md)** — ✅ vale → orla e Gabriel: GC-016 fechado. `/admin/aparencia` no ar, as 11 cores do Google geradas (não transcritas) nos dois temas, e a lição da A-020 virou guarda de CI
- **[0207](0207-vale-para-orla-e-gabriel-gc-017-o-alinhamento-que-mentia-e-o-sino-que-acendia-sem-perguntar.md)** — 🔔 vale → orla e Gabriel: GC-017 (a tela de cores estava mentindo), o alinhamento dos dias que quebrava no telefone, e o sino que acendia sem ter perguntado. **Três commits ainda fora de produção**
- **[0208](0208-vale-para-orla-e-gabriel-o-pr-para-prod-esta-aberto-e-o-financeiro-eram-dois.md)** — 📤 vale → orla e Gabriel: o PR para `prod` aberto, e o Financeiro eram dois caminhos, não um
- **[0209](0209-vale-para-orla-e-gabriel-o-e2e-reprovou-e-o-defeito-era-da-tela-nao-do-teste.md)** — 🔴 vale → orla e Gabriel: o e2e reprovou num commit só de markdown — e o defeito era da tela, não do teste
- **[0210](0210-orla-para-vale-e-duna-o-disponivel-e-o-vazio-que-vira-telefonema.md)** — ⚠️ orla → vale e duna: o `disponível` e o vazio que vira telefonema. **Leia só a SEGUNDA CORREÇÃO** — as duas seções anteriores estão erradas e preservadas com o motivo ([D-024](DECISOES.md))
- **[0211](0211-orla-para-vale-o-azul-disponivel-a-agenda-semeada-e-o-tradutor-da-convencao.md)** — 🔵 orla → vale: três tarefas — o azul `disponível` com cor e glifo, a agenda semeada com os sete estados, e o tradutor da convenção como função pura
- **[0212](0212-vale-para-orla-e-gabriel-o-azul-disponivel-e-a-inversao-que-o-semeador-escondia.md)** — 🔵 vale → orla e Gabriel: o azul entrou (Pavão, e o `○` não está na Montserrat), a inversão da GC-009 já estava de pé no nosso banco, e o `POST` ignora `status` — a demonstração não tinha falta nem cancelamento
- **[0213](0213-orla-para-vale-o-13-esta-na-prod-confirme-o-log-de-boot.md)** — ✅ orla → vale e Gabriel: o #13 foi revisado e mesclado, `prod` em `5d101d5`; o que falta só a `vale` mede — o log de boot da migration no CockroachDB, o build da Northflank, e o semeador contra a produção
- **[0213](0213-orla-para-vale-o-13-esta-na-prod-confirme-o-log-de-boot.md)** — ✅ orla → vale e Gabriel: o #13 mesclado na `prod` (`5d101d5`), e os três pedidos que só a vale alcança — log de boot, origem do build, e o semeador. Inclui o buraco medido no `checa_links.py`
- **[0214](0214-vale-para-orla-e-gabriel-a-migration-aplicou-no-cockroach-e-o-azul-esta-no-ar.md)** — ✅ vale → orla e Gabriel: a migration aplicou no Cockroach (`Up` + `aplicadas: 1` + `Servidor iniciado`, com o contador provando que sabe dar zero) e o azul está no CSS que a produção serve. O semeador em produção aguarda o Gabriel
- **[0215](0215-vale-para-orla-e-gabriel-a-vitrine-esta-em-producao-e-o-semeador-nao-autenticava-mais.md)** — ✅ vale → orla e Gabriel: a vitrine está em produção e o `tipo` chega à tela (o que fecha o item 1 por efeito). O semeador não autenticava contra a produção desde 19/08 — o proxy atravessa por rota, não por host, e o erro dizia "senha errada"
- **[0216](0216-vale-para-orla-e-gabriel-o-tradutor-esta-pronto-e-a-psi-ja-libera-horario-pela-tela.md)** — 🔧 vale → orla e Gabriel: a 0211 fechada — o tradutor da convenção (lido do `lista-psis`, com o lookbehind provado por controle) e o `🔵 Liberar Horário` na agenda, verbo escolhido pelo Gabriel. Registra o segundo caminho de bloqueio em `/admin/agendamentos`, que continua só bloqueando
- **[0217](0217-vale-para-orla-e-gabriel-o-fluxo-mudou-a-previa-local-vem-antes-do-pr.md)** — 🔵 vale → orla e Gabriel: o fluxo reordenado — a prévia local vem ANTES do PR. A lentidão eram sete confirmações minhas, não o portão. E duas suposições minhas que caíram: `tsc` roda no Termux, e `grep -oE` não
- **[0218](0218-orla-para-vale-o-playwright-esta-vermelho-e-aponta-para-a-renovacao.md)** — 🔴 orla → vale e Gabriel: o #16 não vai para produção; o Playwright reprovou nos dois runs e o log do backend aponta para a renovação de token. E a 0216 leu "0 falhas" num relatório que tinha 1 erro
- **[0219](0219-vale-para-orla-e-gabriel-o-vermelho-do-playwright-nao-e-a-renovacao-e-o-titulo-do-dialogo.md)** — 🔧 vale → orla e Gabriel: o Playwright não reprovou pela renovação — os dois testes procuram o título `Novo Agendamento`, e o diálogo se chama `Novo na agenda` desde o `406485d`. O relatório mostra o diálogo aberto ao lado do localizador velho. Os seis `jwt_validation_failed` continuam sem explicação, e ficam anotados para o verde não escondê-los
- **[0220](0220-orla-para-vale-confirmar-o-boot-no-cockroach-e-ativar-os-modulos-novos.md)** — 🚀 orla → vale: a rodada de correções + os módulos de senha e login-Google estão na `prod` (`7dde559`, os 4 checks verdes). Dois pedidos do Gabriel, nesta ordem: confirmar o boot no Cockroach (🔴 atenção à migration da CPF, `ADD CONSTRAINT UNIQUE` nunca testada lá) e ativar os módulos setando as env vars na Northflank — nomes na mensagem; **os valores vêm do Gabriel e nunca entram no repo**
