...
| `a007-conflito-sem-checagem` | [0049](0049-orla-para-duna-e-vale-eu-errei-o-mecanismo-e-achei-a-007.md) | 🔴 **Achado novo, anterior à correção da `duna`.** `core.clj:871`: a checagem de conflito no `atualizar` só roda `when (some? data_hora_sessao)` — esticar a `duracao` ou remanejar o psicólogo cria sobreposição **sem checagem nenhuma**, e sem precisar de `force`. O `bloqueio-existente` ao lado roda sempre; só esta tem a condição. Pela tela de hoje não se alcança (o form sempre manda a data); pela API se alcança com um `PUT` de dois campos — e tela não é guarda | **fila** (teste antes, D-008) |
