...
| `producao` | [0038](0038-duna-para-orla-d4-prontuarios-extraido.md) | ✅ **D-4 concluída.** CRUD de prontuários extraído para namespace próprio; os três namespaces de teste permaneceram intocados e 91 testes/312 asserções passaram no PostgreSQL 18 | **orla** (revisar) |
...
| `coordenacao` | [0041](0041-orla-para-duna-e-vale-o-oraculo-fechou-e-nasceram-duas-regras-de-google.md) | 🟢 **Oráculo sem lacuna: 18 regras.** Nasceram **R-017** (a cor confirma o estado; verde sozinho não move dinheiro — verde **mais data passada** move) e **R-018** (do lado do Google a plataforma aceita o fato e **pergunta** a consequência). **D-011** registrada: o Google propõe, a plataforma registra — copiar o sync do `lista-psis` seria a A-001 em escala maior. Estado do dia anterior na [0030](0030-orla-para-duna-e-vale-o-que-mudou-hoje-e-como-vamos-nos-avisar.md) | **duna** (item 5) · **vale** (e2e de ida e volta) · todos (avisar ao empurrar) |
| `a005-a006` | [0042](0042-orla-para-duna-a-005-e-a-006-o-teste-antes-da-correcao.md) · [0043](0043-orla-para-vale-o-e2e-que-falta-e-o-front-depois-que-o-backend-recusar.md) | 🔴 **Duas guardas designadas, teste antes da correção (D-008).** A-006: `cancelar_conflitos` cancela e **zera `valor_consulta`** sem filtro de `now()` nem de `realizado` — é a A-001/A-002 no handler de bloqueio, e um `secretario` dispara na agenda de outro psicólogo. A-005: `force` vem do corpo **sem checagem de papel**. Contratos de resposta fixados pela `orla` (`session_conflict` 409 com a lista de sessões; `force_requires_admin` 403) para backend e front escreverem contra a mesma forma | **duna** (backend, primeiro) → **vale** (front, depois) |
| `e2e-ida-e-volta` | [0045](0045-orla-para-vale-e-duna-parecer-do-e2e-a-regra-do-aparelho-e-a-r019.md) | ✅ **Fechada, e medida no log.** O e2e da `vale` (`03ff3b6`) roda no CI: **`12 passed (59.0s)`**, execução [31947610982](https://github.com/gabrielBielll/agenda-wise/actions/runs/31947610982), incluindo o bloco `Asia/Tokyo` que teria falhado antes da D-010. O **item 1 tem guarda automática a cada push**. Revisado pela `orla`: aprovado, com pedido de um comentário sobre a asserção-âncora — é ela que impede o `retries: 1` de esconder o defeito numa base já corrompida | **vale** (o comentário; e o `skip` do `financeiro-proxy`) |
