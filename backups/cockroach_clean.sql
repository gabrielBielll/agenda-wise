...
COPY agendamentos (id, clinica_id, paciente_id, psicologo_id, data_hora_sessao, valor_consulta, duracao, status, recorrencia_id, observacoes, valor_repasse, status_repasse, status_pagamento) FROM stdin;
...
COPY bloqueios_agenda (id, clinica_id, psicologo_id, data_inicio, data_fim, motivo, dia_inteiro, created_at, recorrencia_id) FROM stdin;
...
COPY clinicas (id, nome_da_clinica, limite_psicologos) FROM stdin;
967b3f16-04c2-489c-90d4-306bd967844e	Deep Saúde Demo	5
\.



COPY pacientes (id, clinica_id, nome, email, telefone, data_nascimento, endereco, avatar_url, psicologo_id, historico_familiar, uso_medicamentos, diagnostico, contatos_emergencia, status, nota_fiscal, origem, vencimento_pagamento, tipo_pagamento) FROM stdin;
...
COPY papeis (id, nome_papel) FROM stdin;
...
COPY papel_permissoes (papel_id, permissao_id) FROM stdin;
...
COPY permissoes (id, nome_permissao) FROM stdin;
...
COPY prontuarios (id, clinica_id, paciente_id, psicologo_id, data_registro, conteudo, tipo, queixa_principal, resumo_tecnico, observacoes_estado_mental, encaminhamentos_tarefas, agendamento_id, humor) FROM stdin;
...
COPY usuarios (id, clinica_id, papel_id, nome, email, senha_hash, cpf, telefone, data_nascimento, endereco, crp, registro_e_psi, abordagem, area_de_atuacao) FROM stdin;
